...
gom_polygon <- st_polygon(list(matrix(
  c(
    gom_bbox["xmin"],
    gom_bbox["ymin"],
    # Southwest corner
...
gom_sf <- st_sf(name = "Gulf of Mexico",
...
events <- get_event(
  ...
  select(start, lat, lon, vessel_name) |>
  replace_na(replace = list(vessel_flag = "Unknown")) |>
  mutate(month = month(start, label = T))

saveRDS(events, file = here::here("gom_fishing", "www", "2024_GoM_fishing_events.rds"))
