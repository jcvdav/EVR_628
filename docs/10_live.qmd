...
tour_data_clean <- tour_data |>                        
  ...
  mutate(notes = str_to_lower(notes),
         ...
         fauna = str_squish(fauna),                                 # 4) Remove white spsace
...
```{r}
# Nice try... no code this time. But I am using `stat_summary(geom = "col", fun = "sum")`
```
...
ggplot(data = tour_data_clean,
       mapping = aes(x = str_to_sentence(fauna),
                     y = n)) +
