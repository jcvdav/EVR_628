...
You are the lead data analyst at an NGO that works to promote sustainable fisheries.
Your boss asked you what sounds like a simple query:

> What is the value of the bigeye (_Thunnus obesus_) fishery in the purse seine fleet operating in the Eastern Pacific Ocean?

Let's make some assumptions that will help us answer this question:

1. We will interpret "value of the fishery" as revenue
...
7. Save the zip file to **inside** your EVR628 project at: `data/raw/`^[If your web browser didn't allow you to specify the download folder, your file is likely in the "Downloads" folder. Navigate to the downloads folder and copy the ziped folder to the `data/raw/` folder.]
8. Using your finder / explorer, navigate to `EVR628/data/raw/` and unzip the `CatchByFlagGear.zip` file

:::{.callout-important}
# Windows users
You might have to click a button called "Extract" in the top of your explorer window.
:::

9. You will get a new, normal folder called `CatchByFlagGear` under `EVR628/data/raw/`
10. Read the PDF file called `CarchByFlagGear`, which contains some documentation

## Part B) Reading data

0. **Put your post-it up**
1. Start a new script called `tuna_analysis` and save it to your `scripts/02_analysis` folder
...
# Load the data
tuna_data <- read_csv("data/raw/CatchByFlagGear/CatchByFlagGear1918-2024.csv")
...
0. **Put your post-it up**
1. In your _console_, install the `janitor` package using `install.packages("janitor")`
2. Load `janitor` at the top of your script, run that line, and then read the documentation for the `clean_names()` function
...
0. **Put your post-it up**
1. What is the total value?^[Hint: Use `$` and `sum()`]
2. What is the mean annual value?^[Hint: Use `$` and `mean()`]
3. Build a time-series showing revenues by year. Make sure to correctly label the axis and include a title, subtitle, and caption describing the figure and the data source, respectively.
...
ggplot(data = ps_tuna_data,                    # Specify my data
  ...
  labs(x = "Year",                             # Add some labels
       y = "Revenue (M USD)",
       title = "Value of the Bigeye tuna fishery in the Eastern Tropical Pacific",
       subtitle = "The x-axis is year, y-axis is Revenue (mt). Points representa annual totals",
